...
public class PaymentHistoryService {
    ...
    private final ParkingHistoryDAO parkingHistoryDAO = new ParkingHistoryDAO();
    ...
    int totalParkingMinutes; // 총 시간(분)
    ...
    public int calculateTotalCharge(String carNum) { // 총 요금 (total_charge)
        // 정책
        ...
        LocalDateTime exitTime = LocalDateTime.now(); // 출차 시간 (정산 시간 = 현 시간)

        // 입차 시간 - 출차 시간
        Duration duration = Duration.between(entyTime, exitTime);
        totalParkingMinutes = duration.toMinutesPart(); // 총 시간(분)

        // 주차일수
        int dayCount = totalParkingMinutes / 1440;

        if (totalParkingMinutes <= freeTime) {
            totalCharge = 0; // 무료 회차시간 적용 요금
        } else {
            // 24시간 이하 요금 & 24시간 초과시 (24시간 제외 후) 남은 시간에 대한 요금
            int restTimeCharge = ((((totalParkingMinutes % 1440) - basicTime) / extraTime) * extraCharge) + basicCharge;
