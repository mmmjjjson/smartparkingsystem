...
// 출차 모달에서 신규 회원 등록 모달로 전환
window.addEventListener('DOMContentLoaded', function () {
    const params = new URLSearchParams(window.location.search);

    if (params.get('openNewMemberModal') === 'true') {
        const carNum = params.get('carNum');

        if (carNum) {
            const carNumInput = document.getElementById('newCarNumber');
            if (carNumInput && !carNumInput.value) {
                carNumInput.value = decodeURIComponent(carNum);
            }
        }
        ...
        setTimeout(function () {
            ...
            if (modalEl) {
                const newMemberModal = new bootstrap.Modal(modalEl);
                newMemberModal.show();
            }
        }, 300);

        history.replaceState(null, '', window.location.pathname + '?pageNum=' + params.get('pageNum'));
    }
});
